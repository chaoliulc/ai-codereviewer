...
async function main() {
    ...
    let diff = await githubService.getFilteredDiff(eventData, prDetails.owner, prDetails.repo, prDetails.pull_number, core.getInput("exclude").split(",").map((s) => s.trim()));
    if (diff.length === 0) {
    ...
    if (comments.length > 0) {
        await githubService.createReviewComment(prDetails.owner, prDetails.repo, prDetails.pull_number, comments);
    }
}
main().catch((error) => {
